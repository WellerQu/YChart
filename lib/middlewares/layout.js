...
var linkLine = function (nodePool) { return function (lines) {
    return lines.map(function (item) {
        ...
        if (!id.split)
            return item;
        var _a = id.split('-'), source = _a[0], target = _a.slice(1);
        var s = nodePool.find(function (n) { return n.id === source; });
        var t = nodePool.find(function (n) { return n.id === target.join('-'); });
        if (!s || !t)
            return item;
        ...
        var a = Math.atan2(y2 - y1, x2 - x1) * 180 / Math.PI + 90; // 阿尔法a
