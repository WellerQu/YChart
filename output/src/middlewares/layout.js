...
var linkLine = function (nodePool) { return function (lines) {
    return lines.map(function (item) {
        ...
        if (x1 === x2) {
            arrow.data.attrs.d = utils_1.toArrowD(x1, y1 + constants_1.ARROW_OFFSET);
            arrow.data.attrs.transform = "rotate(" + 180 * (y2 > y1 ? 1 : -1) + ", " + x1 + " " + (y1 + constants_1.ARROW_OFFSET + constants_1.ARROW_HEIGHT / 2) + ")";
        }
        else if (y1 === y2) {
            arrow.data.attrs.d = utils_1.toArrowD(x1 + constants_1.ARROW_OFFSET, y1);
            arrow.data.attrs.transform = "rotate(" + 90 * (x2 > x1 ? 1 : -1) + ", " + (x1 + constants_1.ARROW_OFFSET) + " " + y1 + ")";
        }
        else {
            var a = Math.atan2(y2 - y1, x2 - x1) * 180 / Math.PI + 90;
            var k = (y2 - y1) / (x2 - x1);
            var b = y2 - k * x2;
            var arrowX = x1 + constants_1.ARROW_OFFSET;
            var arrowY = k * arrowX + b;
