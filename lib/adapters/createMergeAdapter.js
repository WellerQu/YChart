...
var mergeHTTPOrRPC = function (data) {
    var othersNodes = data.nodes.filter(function (item) { return item.type !== NODE_TYPE_1.NODE_TYPE.HTTP && item.type !== NODE_TYPE_1.NODE_TYPE.RPC; });
    var nodes = data.nodes.filter(function (item) { return item.type === NODE_TYPE_1.NODE_TYPE.HTTP || item.type === NODE_TYPE_1.NODE_TYPE.RPC; });
    var othersLines = data.links.filter(function (item) { return nodes.every(function (node) { return node.id !== item.target; }); });
    var lines = data.links.filter(function (item) { return nodes.some(function (node) { return node.id === item.target; }); });
