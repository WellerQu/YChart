...
exports.moveCanvas = function (stage) { return function (next) { return function (userState) {
    ...
    var handleMouseMove = function (event) {
        ...
        var _b = utils_1.parseViewBoxValue(svgElement.getAttribute('viewBox')), x2 = _b[0], y2 = _b[1], width = _b[2], height = _b[3];
        var containerWidth = svgElement.parentElement.offsetWidth;
        var ratio = -(width / containerWidth);
        var xClamp = utils_1.clamp(-width, width);
        var yClamp = utils_1.clamp(-height, height);
